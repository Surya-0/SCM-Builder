...
class SupplyChainGenerator:
    ...
    def cost_propagation_rm_to_sa(self):
        ...
        self.cost_external_facility_rm = {}  # key: external-facility_node_id value: cost from raw materials
        self.cost_sa_external_facility = {}  # key: subassembly_node_id value: cost from external facilities


    def demand_propagation_rm_to_sa(self):
        self.subassembly_ext_facility = {}  # store the sub assembly parts as keys and external facilities as values
        # dict to store the ef-rm relation
        # key: ef; value: list((rm, quantity))
        self.ext_facility_raw_material = {}

        self.demand_external_facility = {}  # key: external-facility_node_id, value: propagated_demand

        self.demand_rm = defaultdict(int)  # key: rm-part-node_id, value: propagated_demand
        ...
        self.demand_sa = defaultdict(int)  # key: sa-part-node_id, value: propagated demand

    def cost_propagation_sa_to_po(self):
        self.cost_LF = defaultdict(float)  # key : LF id, value : total cost for the facility frm all the sub - assemblies

        self.cost_po = defaultdict(float)  # key : product offering, value : total cost for the offering

    def demand_propagation_sa_to_po(self):
        self.lam_facility_sub_assembly = defaultdict(list)  # store the lam facility as keys and sub assembly id,
        # quantity as values

        self.po_Lam_facility = defaultdict(list)  # Store the product offerings as keys  and Lam facilities as values
        ...
        self.demand_po = defaultdict(int)  # key : product offering id, value : propagated demand

        self.demand_Lam_facility = {}  # key : Facility id, value : calculated demand from formula
    ...
    def initialize_storage(self):
        ...
        self.opcost_facility = {} # key : LF id, value : operation cost for the facility
    ...
    def _generate_facilities(self):
        ...
        for f_type, count in self.facility_distribution.items():
            for _ in range(count):
                ...
                self.opcost_facility[facility_data['id']] = facility_data['operating_cost']
    ...
    def _sa_for_po(self):
        ...
        for product in self.product_offerings:
            ...
            no_of_sa = (self.total_variable_nodes // (10 * self.FIXED_PRODUCT_OFFERINGS)) + 2
    ...
    def _rm_for_sa(self):
        ...
        for sa in sub_assembly:
            sa_id = sa['id']
            no_of_rm = (self.parts_distribution['raw'] // (self.parts_distribution['subassembly'])) + 5
            rm_parts = []
            ...
            for part in subassembly_parts:
                ...
                for rm in raw_materials:
                    ...
                    if facility['id'] in self.ext_facility_raw_material:
                        self.ext_facility_raw_material[facility['id']].append((rm['id'], edge_data['quantity']))
                ...
                for fac in self.facilities['external']:
                    if fac['id'] == facility['id']:
                        ...
                        if part['id'] in self.sum_max_capacity_ext_facility_for_sa:
                            self.sum_max_capacity_ext_facility_for_sa[part['id']] += max_capacity
                        else:
                            self.sum_max_capacity_ext_facility_for_sa[part['id']] = max_capacity

                if part['id'] in self.subassembly_ext_facility:
                    self.subassembly_ext_facility[part['id']].append((facility['id'], max_capacity))
    ...
    def _connect_facilities_to_products(self):
        ...
        for facility in self.facilities['lam']:
            ...
            for product in products:
                ...
                self.po_Lam_facility[product_id].append((facility['id'], facility['max_capacity']))
                self.sum_max_capacity_lam_facility_for_po[product_id] += facility['max_capacity']
        ...
        for product in self.product_offerings:
            upper_bound = max(3, len(self.warehouses['lam']) // 8)
            warehouses = random.sample(self.warehouses['lam'],
                                       max(2, random.randint(3, upper_bound)))
            ...
            for warehouse in warehouses:
                ...
                self._log_edge_operation("create", warehouse['id'], product['id'], edge_data,
        ...
        for family in self.product_families:
            ...
            for offering in self.product_offerings:
                if offering['name'] in PRODUCT_OFFERINGS[family['name']]:
                    offering_id = offering['id']
                    PF_revenue += copy_graph.nodes[offering_id].get('calculated_revenue', 0)

            base_graph.nodes[family_id]['revenue'] = PF_revenue
            print(f"Product Family {family_id}: Revenue = {PF_revenue}")

        business_group_id = self.business_group['id']
        business_group_revenue = sum(
            base_graph.nodes[family['id']].get('revenue', 0) for family in self.product_families)
        base_graph.nodes[business_group_id]['revenue'] = business_group_revenue
        print(f"Business Group {business_group_id}: Total Revenue = {business_group_revenue}")

        self.simulation_graphs[self.simulation_timestamp] = base_graph
    ...
    def simulate_ext_fac_sa_demand(self):
        ...
        for sa, fac_list in self.subassembly_ext_facility.items():
            for fac in fac_list:
                facility_id = fac[0]
                max_capacity = fac[1]
                self.demand_external_facility[facility_id] = (self.demand_sa[sa] /
                                                              self.sum_max_capacity_ext_facility_for_sa[
                                                                  sa]) * max_capacity
    ...
    def simulate_rm_ext_fac(self):
        ...
        for ef, rm_list in self.ext_facility_raw_material.items():
            ...
            for rm in rm_list:
                ...
                self.demand_rm[rm_id] += math.ceil(self.demand_external_facility[ef] * quantity)

                # cost calculation: sum of the cost of all the connected raw materials
                sum_qc_products += quantity * self.cost_rm[rm_id]

            self.cost_external_facility_rm[ef] = (self.demand_external_facility[ef] * sum_qc_products) + self.opcost_facility[ef]  # need to add operating cost
        ...
        for po, fac_list in self.po_Lam_facility.items():
            for fac in fac_list:
                ...
                self.demand_Lam_facility[facility_id] = (
                            self.demand_po[po] * max_capacity / self.sum_max_capacity_lam_facility_for_po[po])

    def simulate_sa_lam_fac(self):
        """
        ...
        for lf, sa_list in self.lam_facility_sub_assembly.items():
            for sa in sa_list:
                ...
                self.demand_sa[sa_id] += math.ceil(self.demand_Lam_facility[lf] * quantity)
    ...
    def export_to_csv(self, graph_dict, export_dir='exports'):
        ...
        for period, graph in graph_dict.items():
            ...
            metadata = {
                ...
                'lam_warehouse_count': len([n for n, d in graph.nodes(data=True) if
                                            d.get('node_type') == 'warehouse' and d.get('type') == 'lam']),
                'supplier_warehouse_count': len([n for n, d in graph.nodes(data=True) if
                                                 d.get('node_type') == 'warehouse' and d.get('type') == 'supplier']),
                'subassembly_warehouse_count': len([n for n, d in graph.nodes(data=True) if
                                                    d.get('node_type') == 'warehouse' and d.get(
                                                        'type') == 'subassembly']),

                'facilities_count': len([n for n, d in graph.nodes(data=True) if d.get('node_type') == 'facility']),

                'external_facility_count': len([n for n, d in graph.nodes(data=True) if
                                                d.get('node_type') == 'facility' and d.get('type') == 'external']),
                'lam_facility_count': len([n for n, d in graph.nodes(data=True) if
                                           d.get('node_type') == 'facility' and d.get('type') == 'lam']),

                'parts_count': len([n for n, d in graph.nodes(data=True) if d.get('node_type') == 'part']),
                'raw_part_count': len(
                    [n for n, d in graph.nodes(data=True) if d.get('node_type') == 'part' and d.get('type') == 'raw']),
                'subassembly_part_count': len([n for n, d in graph.nodes(data=True) if
                                               d.get('node_type') == 'part' and d.get('type') == 'subassembly']),
